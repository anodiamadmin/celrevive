...
export default function CapturePhoto({ onSubmit }) {
  ...
  const confirmCrop = () => {
    // [UPDATED]: 100% select par direct bypass taaki size kam na ho
    ...
    // [UPDATED]: object-contain ke liye math update kiya gaya
  ...
  //     // [CHANGED]: URL updated to /image-analysis
  ...
  //     // [CHANGED]: Foolproof validation check applied here
  //     const imagePassed = data.is_valid === true || data.valid === true;

  //     if (!res.ok || !imagePassed) {
  ...
  //     setValidationError('Failed to connect to server. Check your internet.');
  //     setCapturedImage(null);
  //     setStage('idle');
  //     return { isValid: false };
  //   } finally {
...
//       // Backend ke log se match karta hua session ID ya random ID
//       const sessionId = "5581e1d0-4e59-448c-b5a3-1187c05c2365"; // ya crypto.randomUUID()
//       formData.append('image', imageFile);
//       formData.append('session_id', sessionId);
...
//       // [FIXED]: Agar backend ne 'image_skin_concerns' bhej diya hai, 
//       // iska matlab API request successful thi aur image analyze ho chuki hai!
//       if (!res.ok) {
//         setValidationError(data.message || 'Image analysis failed. Please try again.');
//         setCapturedImage(null); 
//         setStage('idle'); 
//         return { isValid: false };
//       }

//       // Agar data successfully aa gaya, toh isValid true karke sessionId bhej do
//       return { isValid: true, sessionId: data.session_id || sessionId };
      
//     } catch (error) {
//       console.error('Image analysis API failed:', error);
//       setValidationError('Failed to connect to server. Check your internet or CORS setup.');
//       setCapturedImage(null);
//       setStage('idle');
//       return { isValid: false };
//     } finally {
...
const validateImage = async (imageData) => {
    ...
    try {
      ...
      console.log("Response Status:", res.status); // 👈 Yeh check karega ki status kya hai

      // Agar response OK nahi hai toh yahin rok do
      if (!res.ok) {
     ...
     return { isValid: true, sessionId, backendData: data };
      ...
      // 🚨 YAHAN DEKHO ASLI ERROR KYA HAI
  ...
  const submitPhoto = async () => {
    ...
    // if (result.isValid) {
    //   onSubmit?.(capturedImage, result.sessionId);
    // }
    if (result.isValid) {
